...
def process_page(link):
    while True:
        try:
            ...
            # if response.status_code == 200:  # Page successfully fetched
            ...
            break
            # else:
            #     logging.warning(f"Failed to fetch link: {link}. Retrying...")
        except IndexError:  # Исключение, которое вы хотите обработать
            logging.warning(f"No contact information found for {link}. Retrying...")
            time.sleep(1)  # Подождите перед повторной попыткой
        except Exception as e:
...
with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
    res = executor.map(lambda link: process_page(link), current_web_links)
end_time = time.time()
